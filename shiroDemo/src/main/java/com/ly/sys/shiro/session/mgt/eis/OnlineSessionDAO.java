...
package com.ly.sys.shiro.session.mgt.eis;
...
import com.ly.sys.core.user.service.OperatorOnlineService;
import com.ly.sys.core.user.vo.OperatorOnline;
import com.ly.sys.shiro.ShiroConstants;
import com.ly.sys.shiro.session.mgt.OnlineSessionFactory;
...
public class OnlineSessionDAO extends EnterpriseCacheSessionDAO {
    ...
    protected Session doReadSession(Serializable sessionId) {
        OperatorOnline userOnline = operatorOnlineService.findById(String.valueOf(sessionId));
        if (userOnline == null) {
            return null;
        }
    ...
    public void syncToDb(OnlineSession session) {
        ...
        if (lastSyncTimestamp != null) {
            ...
            boolean isGuest = session.getUserId() == null || session.getUserId().equals("") || session.getUserId().equals("anonymous");
        ...
        operatorOnlineService.online(OperatorOnline.fromOnlineSession(session));
    ...
    protected void doDelete(Session session) {
        ...
        if (onlineSession.getAttribute(ShiroConstants.ONLY_CLEAR_CACHE) == null) {
            try {
              operatorOnlineService.offline(String.valueOf(onlineSession.getId()));
            } catch (Exception e) {
                //即使删除失败也无所谓
            }
        }

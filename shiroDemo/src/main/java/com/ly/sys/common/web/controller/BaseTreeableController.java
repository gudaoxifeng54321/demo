...
import com.ly.sys.common.enums.BooleanEnum;
import com.ly.sys.common.plugin.entity.Treeable;
import com.ly.sys.common.plugin.entity.ZTree;
import com.ly.sys.common.service.BaseTreeableService;
import com.ly.sys.common.web.controller.permission.PermissionList;

public abstract class BaseTreeableController<M extends AbstractEntity<ID> & Treeable<ID>, ID extends Serializable> extends
    BaseController<M, ID> {
  ...
  public String tree(HttpServletRequest request, @RequestParam(value = "searchName", required = false) String searchName,
      @RequestParam(value = "async", required = false, defaultValue = "false") boolean async, Searchable searchable, Model model) {
  ...
  public String update(Model model, @ModelAttribute("m") M m, BindingResult result, RedirectAttributes redirectAttributes) {
    ...
    // 保存前检查是否有子节点
    List<M> allChilds = baseService.findChildNode(m.getId());
    if (allChilds.size() > 0) {
      m.setIsParent("1");
    }
  ...
  public String deleteInBatch(@RequestParam(value = "ids", required = false) ID[] ids,
    ...
    for (ID id : ids) {
      List<M> resList = baseService.findAllWithNoPageNoSort(Searchable.newSearchable().addSearchParam("id", id));
  ...
  public String appendChildForm(@PathVariable("parent") ID parentId, Model model) {
    ...
    model.addAttribute("parent", baseService.findById(parentId));
  ...
  public String appendChild(Model model, @PathVariable("parent") ID parentId, @ModelAttribute("child") M child, BindingResult result,
    ...
    M m = baseService.findById(parentId);
  ...
  public String showMoveForm(HttpServletRequest request,
      @RequestParam(value = "async", required = false, defaultValue = "false") boolean async, @PathVariable("source") ID sourceId,
      Searchable searchable, Model model) {

    if (this.permissionList != null) {
      this.permissionList.assertHasEditPermission();
    }
    M source = baseService.findById(sourceId);
    List<M> models = null;
    if (!async) {
      models = baseService.findAllWithSort(searchable);
    } else {
    ...
    List<M> newModels = new ArrayList<>();
    // 排除自己及子子孙孙
    List<M> childNodes = baseService.findChildNode(sourceId);
    for (M m : models) {
      if (m.getId().equals(sourceId)) {
        continue;
      }
      if (childNodes.contains(m)) {
        continue;
      }
      newModels.add(m);
    }

    model.addAttribute("trees", convertToZtreeList(request.getContextPath(), newModels, async, true));
  ...
  @PageableDefaults(sort = { "parent_ids=asc", "weight=asc" })
  public String move(HttpServletRequest request, @RequestParam(value = "async", required = false, defaultValue = "false") boolean async,
      @PathVariable("source") ID sourceId, @RequestParam("target") ID targetId, @RequestParam("moveType") String moveType,
      Searchable searchable, Model model, RedirectAttributes redirectAttributes) {

    if (this.permissionList != null) {
      this.permissionList.assertHasEditPermission();
    }
    M target = baseService.findById(targetId);
    M source = baseService.findById(sourceId);
    if (target.isRoot() && !moveType.equals("inner")) {
      model.addAttribute(Constants.ERROR, "不能移动到根节点之前或之后");
      return showMoveForm(request, async, sourceId, searchable, model);
    }
  ...
  @PageableDefaults(sort = { "parent_ids=asc", "weight=asc" })
  public String list(HttpServletRequest request, @PathVariable("parentId") ID parentId, Searchable searchable, Model model)
    ...
    if (parentId != null) {
      searchable.addSearchParam("parentId", parentId);
    }
    model.addAttribute("parent", baseService.findById(parentId));
  ...
  @PageableDefaults(sort = { "parent_ids=asc", "weight=asc" })
  public String listTable(HttpServletRequest request, @PathVariable("parentId") ID parentId, Searchable searchable, Model model)
      throws UnsupportedEncodingException {

    list(request, parentId, searchable, model);
  ...
  public Object load(HttpServletRequest request, @RequestParam(value = "async", defaultValue = "true") boolean async,
      ...
      @RequestParam(value = "searchName", required = false) String searchName, @RequestParam(value = "id", required = false) ID parentId,
    ...
    M excludeM = baseService.findById(excludeId);

    List<M> models = null;

    if (!StringUtils.isEmpty(searchName)) {// 按name模糊查
      searchable.addSearchParam("name", searchName);
      ...
      if (async && !asyncLoadAll) { // 异步模式下 且非异步加载所有
        // 排除自己 及 子子孙孙
        if (excludeM != null) {
          models = baseService.addExcludeSearchFilter(models, excludeM);
        }
      }

    }
    return convertToZtreeList(request.getContextPath(), models, async && !asyncLoadAll && parentId != null, onlyCheckLeaf);
  }

  @RequestMapping(value = "ajax/{parent}/appendChild", method = RequestMethod.GET, produces = "application/json")
  @ResponseBody
  public Object ajaxAppendChild(HttpServletRequest request, @PathVariable("parent") ID parentId) {
  ...
  public Object ajaxDeleteSelfAndChildren(@PathVariable("id") ID id) {
    ...
    M tree = baseService.findById(id);
  ...
  @ResponseBody
  public Object ajaxRename(HttpServletRequest request, @PathVariable("id") ID treeId, @RequestParam("newName") String newName) {

    if (permissionList != null) {
      permissionList.assertHasUpdatePermission();
    }
    M tree = baseService.findById(treeId);
  ...
  @ResponseBody
  public Object ajaxMove(@PathVariable("sourceId") ID sourceId, @PathVariable("targetId") ID targetId,

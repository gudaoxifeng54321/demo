 ...
 * 
 * 广告系统redis和连接池的封装 Jedis操作步骤如下： 获取Jedis实例需要从JedisPool中获取； 用完Jedis实例需要返还给JedisPool； 如果Jedis在使用过程中出错，则也需要还给JedisPool；
 * 
 * @author mtwu
 * @since 2016-11-25
 */
...
public class ADRedisUtil {
  private final static Logger log  = LoggerFactory.getLogger(ADRedisUtil.class);
  private static JedisPool    pool = null;
	public static String ip = X.getConfig("domain.admgt.redis.url");
	public static Integer port = Integer.parseInt(X.getConfig("domain.admgt.redis.port"));
	public static Integer database = Integer.parseInt(X.getConfig("domain.admgt.redis.database"));

	public static JedisPool getPool(){
	  if (pool == null) {
	    createPool(ip, port,database);
	  }
	  return pool;
	}
  /**
   ...
   * @return JedisPool
   */
  public static JedisPool createPool(String ip, int port, int database) {
    if (pool == null) {
  ...
  public static String get(String key) {
    ...
    try {
    	if(pool==null){
    		pool = createPool(ip, port,database);
    	}
  ...
  public static long zadd(String key, Map<String, Double> scoreMembers){
    ...
    long result = 0l;
    try {
      jedis = ADRedisUtil.getJedis();
      result = jedis.zadd(key, scoreMembers);
    } catch (Exception e) {
       log.error("jedis zadd Error key["+ key +"] scoreMembers["+scoreMembers+"]",e);
      pool.returnBrokenResource(jedis);
    } finally {
      pool.returnResource(jedis);
    }
  ...
  public static long zunionstore(String dstkey, String... sets){
    ...
    try {
      ...
      result = jedis.zunionstore(dstkey, sets);
       ...
       log.error("jedis zunionstore Error dstkey["+ dstkey +"] sets["+sets.toString()+"]",e);
      pool.returnBrokenResource(jedis);
    } finally {
      pool.returnResource(jedis);
    }
  ...
  public static Set<String> zrangeByScore(String key, double min, double max){
    ...
    try {
      ...
      result = jedis.zrangeByScore(key, min, max);
       ...
       log.error("jedis zrangeByScore Error key["+ key +"] min["+min+"] max["+max+"]",e);
      pool.returnBrokenResource(jedis);
    } finally {
      pool.returnResource(jedis);
    }
  ...
  public static Object eval(String script, List<String> keys, List<String> args){
    ...
    try {
      ...
      result = jedis.eval(script, keys, args);
       ...
       log.error("jedis lua script Error ["+script+"] params["+ keys +"] args["+args+"]",e);
      pool.returnBrokenResource(jedis);
    } finally {
      pool.returnResource(jedis);
    }
